---
capability: image-temp-hosting
name: 图片临时公网托管
description: 聊天附件图片上传到免费临时图床 litterbox，生成公网可访问的图片直链，供前端展示与 qwen-image / 视频模型推理使用
---

## Purpose

为本地不公网部署的项目提供零成本公网图片托管：上传图片后返回公网可访问的临时直链，使前端消息气泡可展示缩略图、模型可通过公网拉取图片完成以图生图/生视频推理。图片 72h 后自动过期。
