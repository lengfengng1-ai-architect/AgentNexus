...
- **GIVEN** `brand_input.selected_cities` 为 `["上海", "成都"]`
...
`plan_data_query` 节点 SHALL 读取 `brand_input.selected_cities`（数组），对每座城市调用 `get_city_data(city)` 查询，将各城结果聚合为 `MultiCityDataOutput { cities: [CityDataOutput] }`，数组顺序 SHALL 与 `selected_cities` 输入顺序一致（主城 `selected_cities[0]` 在首位）。`brand_input.selected_cities` 为空或缺失时 SHALL 退化为 `[brand_input.city]` 单城以保持向后兼容。

#### Scenario: 多城遍历输出聚合结构
- **GIVEN** `brand_input.selected_cities` 为 `["上海", "成都", "广州"]`
- **WHEN** `plan_data_query` 节点执行
...
#### Scenario: selected_cities 为空时退化为单城
- **GIVEN** `brand_input.selected_cities` 为空或缺失，`brand_input.city` 为 "上海"
- **WHEN** `plan_data_query` 节点执行
- **THEN** 节点 SHALL 以 `["上海"]` 作为查询城市列表
- **AND** 输出 `cities` 数组长度 SHALL 为 1，行为等价单城现状

#### Scenario: 某城市无 mock 数据时报错
- **GIVEN** `selected_cities` 含某座 mock 未覆盖的城市
- **WHEN** `plan_data_query` 遍历到该城市
- **THEN** 节点 SHALL 抛出错误并列出 `list_cities()` 返回的可用城市

### Requirement: strategy_generation 节点 SHALL 分配差异化城市角色

`strategy_generation` 节点 SHALL 基于 `brand_input` 与 `plan_data_query` 的多城数据，由 LLM 为每座城市分配差异化的战略角色定位（枚举示例：`flagship_launch` 旗舰首发 / `experience_cultivation` 体验深耕 / `channel_conversion` 渠道转化 / `community_growth` 社群裂变），主城（`selected_cities[0]`）SHALL 承担 `flagship_launch` 角色。角色分配 SHALL 引用各城数据作为依据，属 LLM 推理结论 / 创意策划（`llm_boundaries.can_generate`）。

#### Scenario: 多城分配差异化角色
- **GIVEN** `selected_cities` 为 `["上海", "成都", "广州"]`
...
- **GIVEN** `selected_cities` 为多城，`plan_data_query` 输出多城数据
