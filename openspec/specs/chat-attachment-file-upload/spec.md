---
capability: chat-attachment-file-upload
name: 聊天附件本地文件上传+预览
description: 聊天输入框附件从 URL 输入改造为本地文件选取/拖拽上传，支持图片预览和文件名+后缀展示，发送时自动上传到后端获取 URL
---

## Purpose

将聊天附件的交互方式从手动输入图片 URL 改为本地文件选取+拖拽上传，降低使用门槛，同时保留 textarea 中输入 URL 的自动识别能力。
