...
> 锚定 `superpowers.yaml` in_scope `plan-generation`（工作台主体），次要引用 `brand-input`（① 对话入口）。
> 本变更（P1）只覆盖：页面可达 + 纯白居中手机框 + Tab 切换（① 实现、②-⑤ 占位）+ ① 聊天交互 + 样式隔离 + 路由后退/前进。②-⑤ 完整行为由后续变更以 ADDED Requirements 补齐。

## Purpose

提供 `/mobile` 移动端工作台展示页，在纯白背景上居中显示一个手机界面框架，通过 ①-⑤ Tab 切换 5 屏（对话入口 / 简报 / 方案生成 / 行动建议 / 下发转发达成），作为移动端形态的设计参考与可视化预览。样式与 PC 端隔离、自成一套。
...
系统 SHALL 提供一个 Tab 切换器，展示 5 个屏幕标签（① 对话入口 / ② 简报 / ③ 方案生成 / ④ 行动建议 / ⑤ 下发转发达成），点击切换手机框架内显示的屏幕内容。
