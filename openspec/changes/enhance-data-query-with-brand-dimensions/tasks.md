...
- [x] 7.3 覆盖率 ≥ 80%（22 tests, 0 failed）

## 8. 向后兼容

- [x] 8.1 验证旧 Workflow（不含 brand_name/category 的）仍可正常运行（Legacy 3 tests pass）
- [x] 8.2 验证 `allygo_city_data.json` 仍可被 `get_city_data()` 正常读取
