...
const ITEMS: { id: string; icon: () => JSX.Element; title: string; subtitle: string; onClick: keyof Omit<ChatActionPanelProps, 'isOpen' | 'onClose'> }[] = [
  ...
  { id: 'clear', icon: ClearIcon, title: '清空聊天记录', subtitle: '重置对话上下文', onClick: 'onClearHistory' },
]

export function ChatActionPanel({ isOpen, onClose, onUpload, onCreateImage, onCreateVideo, onClearHistory }: ChatActionPanelProps) {
  if (!isOpen) return null

  const handlers = { onUpload, onCreateImage, onCreateVideo, onClearHistory }
