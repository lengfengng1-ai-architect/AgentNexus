...
const HIDE_TABS: MobileScreen[] = ['preview', 'budget-preview', 'action-preview', 'research-report', 'budget-assessment', 'activity-planning', 'alliance-planning', 'competitor-analysis', 'community-operations', 'node-preview']
...
export function MobileWorkbenchPage() {
  ...
  // Budget assessment overlay state
  ...
  // Activity planning overlay state
  const [activityPlanningId, setActivityPlanningId] = useState<string | null>(null)
  const [isApAnimatingOut, setIsApAnimatingOut] = useState(false)
  const [activityPlanningTitle, setActivityPlanningTitle] = useState('')

  // Alliance planning overlay state
  ...
  const handleViewActionResult = () => {
    ...
    const ar = ((outputs as Record<string, unknown>).action_recommendations ?? (outputs as Record<string, unknown>)._actions ?? snapshotAr) as Record<string, unknown> | undefined
    ...
    const brandInput = ((outputs as Record<string, unknown>).brand_input ?? snapshotBrand) as Record<string, unknown> | undefined
    const budgetKpi = ((outputs as Record<string, unknown>).budget_kpi ?? (outputs as Record<string, unknown>)._budget ?? snapshotBudget) as Record<string, unknown> | undefined
    if (ar && Array.isArray(ar.actions)) {
      setActionPreviewData({
        actions: ar.actions as { title: string; description: string; start_date?: string; end_date?: string; priority?: string; category?: string }[],
        brandName: (brandInput?.brand_name as string) || '',
  ...
  const handleChatNavigate = (s: MobileScreen, inputText?: string, brandInput?: BrandInput, researchId?: string, budgetAssessmentIdParam?: string, activityPlanningIdParam?: string, alliancePlanningIdParam?: string, caIdParam?: string, coIdParam?: string) => {
    ...
    if (s === 'activity-planning') {
      if (!activityPlanningIdParam) return
      setActivityPlanningId(activityPlanningIdParam)
      setActivityPlanningTitle(inputText || '')
      setScreen('activity-planning')
      return
    }
    if (s === 'alliance-planning') {
      if (!alliancePlanningIdParam) return
      setAlliancePlanningId(alliancePlanningIdParam)
      setAlliancePlanningTitle(inputText || '')
  ...
  // 活动规划详情页返回：滑出动画 → 300ms 后卸载回聊天屏
  const handleActivityBack = () => {
    setIsApAnimatingOut(true)
    setTimeout(() => {
      setIsApAnimatingOut(false)
      setActivityPlanningId(null)
      setScreen('chat')
    }, 300)
  }

  // 盟域规划详情页返回：滑出动画 → 300ms 后卸载回聊天屏
  const handleAllianceBack = () => {
    setIsAlAnimatingOut(true)
    setTimeout(() => {
      setIsAlAnimatingOut(false)
      setAlliancePlanningId(null)
      setScreen('chat')
    }, 300)
  }
  ...
  const hideTopbar = screen === 'budget-preview' || isBpAnimatingOut || screen === 'action-preview' || screen === 'research-report' || screen === 'draft-list' || screen === 'budget-assessment' || screen === 'activity-planning' || screen === 'alliance-planning' || screen === 'competitor-analysis' || screen === 'community-operations' || screen === 'node-preview' || nodePreviewAnimatingOut
  ...
  // 活动规划覆盖屏展示中（含滑出动画期间）
  const showingActivityPlanning = screen === 'activity-planning' || isApAnimatingOut
    ...
    <div className="mw">
      ...
      <div style={{ position: 'relative' }}>
        <PhoneFrame topbar={hideTopbar ? undefined : topbar}>
          ...
          {/* 活动规划详情覆盖层 */}
          {showingActivityPlanning && activityPlanningId && (
            <div
              className={isApAnimatingOut ? 'activity-slide-out' : 'activity-slide-in'}
              style={{
                position: 'absolute', inset: 0, zIndex: 40,
                display: 'flex', flexDirection: 'column',
                background: 'var(--bg)', overflow: 'hidden',
            ...
            >
              <ScreenActivityPlanning
                activityId={activityPlanningId}
                fallbackTitle={activityPlanningTitle}
                onBack={handleActivityBack}
              />
            </div>
          )}
          {/* 盟域规划详情覆盖层 */}
          {showingAlliancePlanning && alliancePlanningId && (
            <div
              className={isAlAnimatingOut ? 'alliance-slide-out' : 'alliance-slide-in'}
              style={{
                position: 'absolute', inset: 0, zIndex: 40,
                display: 'flex', flexDirection: 'column',
                background: 'var(--bg)', overflow: 'hidden',
            ...
            >
              <ScreenAlliancePlanning
                allianceId={alliancePlanningId}
                fallbackTitle={alliancePlanningTitle}
                onBack={handleAllianceBack}
              />
            </div>
          )}
          {/* 竞品分析详情覆盖层 */}
