...
def patch_chat_pipeline_agents(monkeypatch):
    ...
    registry.register("market_analysis", lambda state: {"result": {}})
    registry.register("requirement_collector", lambda state: {"is_complete": True, "missing_fields": [], "brand_input": state.get("brand_input", {})})
    registry.register("market_research", lambda state: {"market_summary": "", "trends": [], "opportunities": []})
    ...
    registry.register("action_recommendations", lambda state: {"actions": []})
    registry.register("plan_generator", lambda state: {"chapters": []})
