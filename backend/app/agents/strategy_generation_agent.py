...
_MOCK_PATH = Path(__file__).parent.parent.parent / "mock_data" / "plan_strategy.json"
_PROMPT_DIR = Path(__file__).parent.parent / "prompt_templates"


def _load_mock() -> StrategyOutput:
    with _MOCK_PATH.open("r", encoding="utf-8") as f:
        return StrategyOutput.model_validate(json.load(f))
